#!/bin/bash
# ci_bench_wrr_weight.sh — WRR (Weighted Round Robin) scheduler E2E benchmark
#
# Four tests on a 4-path veth topology. T1 uses UDP DL (server→client) to test
# bandwidth aggregation the same way as WRTT. T2/T3/T4 use UDP UL (client→
# server) so the client's WRR + weight settings drive path selection; per-
# interface TX-byte counters on the client veths make the *proportional*
# distribution directly observable.
#
# Unlike WRTT (winner-take-all: the single highest-weight path drains until
# cwnd-blocked), WRR interleaves every usable path continuously in proportion
# to weight and never looks at RTT for datagram traffic. So the pass criteria
# here check that each path's share of TX bytes tracks its weight/total_weight
# ratio (within a tolerance band), not that one path dominates — T4 in
# particular expects an even ~25/25/25/25 split despite very different RTTs,
# which is the opposite of what WRTT's RTT-tiebreak test (T4) expects.
#
#   T1  Aggregation       UDP DL at 600 Mbps target — exceeds path A's 300 Mbps
#                         cap so WRR must spill over to B/C/D.
#                         4-path WRR throughput must be >= 120% of 1-path.
#
#   T2  Weight ratio      Weights: A=3 B=C=D=1 (total=6). Low target BW
#                         (100 Mbps, well below A's 300 Mbps cap). Path A's
#                         share of client TX bytes must land within
#                         expected(50%) +/- 15pp, and be the largest share.
#
#   T3  Weight > RTT      Weights: B=3 A=C=D=1 (total=6). Low target BW
#                         (40 Mbps, below B's 80 Mbps cap). Path B has the
#                         worst RTT (60 ms) but its share must still land
#                         within expected(50%) +/- 15pp and exceed A's share —
#                         proving weight drives the split, not RTT.
#
#   T4  Equal split       All weights=1. Target 100 Mbps (below path A's
#                         300 Mbps cap). Every path's share must land within
#                         expected(25%) +/- 15pp — WRR ignores RTT entirely
#                         for datagram traffic, so no single path may dominate.
#
# Paths (one-way delay / rate; RTT = 2 x delay):
...
#
# Output:  ci_bench_results/wrr_weight_<timestamp>.json
# Usage:   sudo ./ci_bench_wrr_weight.sh [path-to-mqvpn-binary] [--log-level LEVEL]
...
# T1: UDP DL at a target that exceeds path A's cap → WRR must spill to B/C/D
IPERF_AGG_DURATION=20
IPERF_AGG_BW="600M"    # exceeds path A's 300 Mbps cap; forces spillover
...
IPERF_EQUAL_BW="100M"      # T4: equal weights, below path A cap
IPERF_WEIGHT_DURATION=20

AGGREGATION_PASS_THRESHOLD=120   # 4-path >= 120% of single-path
SHARE_TOLERANCE_PP=15            # actual share must be within +/- this of the
                                  # weight/total_weight ratio, in percentage points
...
_wr_cleanup() {
    [ -n "$_WR_CLIENT_PID" ] && { kill "$_WR_CLIENT_PID" 2>/dev/null || true; wait "$_WR_CLIENT_PID" 2>/dev/null || true; }
    [ -n "$_WR_SERVER_PID" ] && { kill "$_WR_SERVER_PID" 2>/dev/null || true; wait "$_WR_SERVER_PID" 2>/dev/null || true; }
    _WR_CLIENT_PID="" ; _WR_SERVER_PID=""
    ...
    [ -n "$_WR_WORK_DIR" ] && rm -rf "$_WR_WORK_DIR" || true
}
trap _wr_cleanup EXIT
...
_wr_start_server() {
    ip netns exec "$NS_SERVER" "$MQVPN" \
        ...
        --subnet 10.0.0.0/24 \
        --cert "${_WR_WORK_DIR}/server.crt" \
        --key  "${_WR_WORK_DIR}/server.key" \
        --auth-key "$_WR_PSK" \
        --scheduler wrr \
        --mtu 1350 \
        --cc "$CI_BENCH_CC" \
        --log-level "$LOG_LEVEL" >"${_WR_WORK_DIR}/server.log" 2>&1 &
    _WR_SERVER_PID=$!
    sleep 2
    if ! kill -0 "$_WR_SERVER_PID" 2>/dev/null; then
        echo "ERROR: server died"; cat "${_WR_WORK_DIR}/server.log" >&2; return 1
    fi
}
...
_wr_start_client() {
    ...
    if [ -n "$_WR_CLIENT_PID" ] && kill -0 "$_WR_CLIENT_PID" 2>/dev/null; then
        kill "$_WR_CLIENT_PID" 2>/dev/null || true
        wait "$_WR_CLIENT_PID" 2>/dev/null || true
        _WR_CLIENT_PID=""
        sleep 1
    fi
    ...
    ip netns exec "$NS_CLIENT" "$MQVPN" \
        ...
        --insecure \
        --log-level "$LOG_LEVEL" >"${_WR_WORK_DIR}/client.log" 2>&1 &
    _WR_CLIENT_PID=$!
    sleep 3
    if ! kill -0 "$_WR_CLIENT_PID" 2>/dev/null; then
        echo "ERROR: client died"; cat "${_WR_WORK_DIR}/client.log" >&2; return 1
    fi
}
...
# complete → XQC_PATH_STATE_ACTIVE, required by WRR and xqc_conn_set_path_weight.
...
# ── T1: UDP DL aggregation (target exceeds single-path cap → WRR must spill) ──

run_t1_aggregation() {
    echo ""
    echo "================================================================"
    echo "  T1: WRR 4-path aggregation (UDP DL ${IPERF_AGG_BW}, server→client)"
    echo "================================================================"
    ...
    # 4-path — 600 Mbps target exceeds path A's 300 Mbps cap; WRR spills to
    # C (200 Mbps), D (150 Mbps), B (80 Mbps) proportionally once A saturates.
    _wr_start_client "$paths4"
    ci_bench_wait_tunnel 20
...
# ── Proportional weight distribution test (T2/T3/T4) ──
#
# Args: label  description  check_iface  expected_share_pct  udp_bw_target  [iface=weight ...]
# Measures client veth TX bytes before/after a UDP upload test and checks
# that check_iface's share lands within +/- SHARE_TOLERANCE_PP of
# expected_share_pct (the iface's weight / total_weight ratio).

run_weight_dist_test() {
    local label="$1" desc="$2" check_iface="$3" expected_pct="$4" bw_target="$5"
    shift 5
    ...
    # Compute byte deltas and check_iface's share vs. expected ratio
    local py_result
    py_result=$(python3 -c "
...
chk_idx = ifaces.index('$check_iface')
chk_sh  = delta[chk_idx] / total * 100
expected = $expected_pct
tol      = $SHARE_TOLERANCE_PP
is_max   = delta[chk_idx] == max(delta)

for ifc, d in zip(ifaces, delta):
    print(f'tx_{ifc}={d}')
    print(f'share_{ifc}={d/total*100:.1f}')
print(f'check_share={chk_sh:.1f}')
print(f'expected_share={expected:.1f}')
print(f'is_max_share={str(is_max).lower()}')
print(f'passed={str(abs(chk_sh - expected) <= tol).lower()}')
")

    echo "$py_result" | while IFS='=' read -r k v; do printf '  %s = %s\n' "$k" "$v"; done

    local chk_share passed
    chk_share=$(echo "$py_result" | grep '^check_share=' | cut -d= -f2)
    passed=$(echo    "$py_result" | grep '^passed='      | cut -d= -f2)

    echo "  ${label}: check=${check_iface} share=${chk_share}%  expected=${expected_pct}%+-${SHARE_TOLERANCE_PP}pp  => $([ "$passed" = "true" ] && echo PASS || echo FAIL)"

    # Export to globals
    eval "_${label}_PASSED=${passed}"
    eval "_${label}_CHK_SHARE=${chk_share}"
...
echo "================================================================"
echo "  mqvpn WRR (Weighted Round Robin) Scheduler E2E Benchmark"
echo "  Binary:  $MQVPN"
...
echo "    T2  Weight ratio: UDP UL ${IPERF_WEIGHT_A_BW}, A=3 B=C=D=1 -> A expected ~50% share"
echo "    T3  Weight > RTT: UDP UL ${IPERF_WEIGHT_B_BW}, B=3 A=C=D=1 -> B expected ~50% despite worst RTT"
echo "    T4  Equal split:  UDP UL ${IPERF_EQUAL_BW}, all weights=1 -> each path expected ~25% (no RTT bias)"
echo "================================================================"

_wr_setup_netns
_wr_apply_netem

_WR_WORK_DIR="$(mktemp -d)"
_WR_PSK=$("$MQVPN" --genkey 2>/dev/null)
openssl req -x509 -newkey ec -pkeyopt ec_paramgen_curve:prime256v1 \
    -keyout "${_WR_WORK_DIR}/server.key" -out "${_WR_WORK_DIR}/server.crt" \
    -days 1 -nodes -subj "/CN=ci-wrr-weight" 2>/dev/null
...
# ── T2: Path A (weight=3 of 6) expected ~50% share ──
_T2_PASSED="false"; _T2_CHK_SHARE="0.0"; _T2_MBPS="0.0"; _T2_RESULT=""
run_weight_dist_test \
    "T2" "Weight ratio: A=3 B=C=D=1, target ${IPERF_WEIGHT_A_BW} UDP UL" \
    "$VETH_A0" "50.0" "$IPERF_WEIGHT_A_BW" \
    "${VETH_A0}=3" "${VETH_B0}=1" "${VETH_C0}=1" "${VETH_D0}=1"

# ── T3: Path B (weight=3 of 6, worst RTT) expected ~50% share ──
_T3_PASSED="false"; _T3_CHK_SHARE="0.0"; _T3_MBPS="0.0"; _T3_RESULT=""
run_weight_dist_test \
    "T3" "Weight > RTT: B=3 A=C=D=1, target ${IPERF_WEIGHT_B_BW} UDP UL" \
    "$VETH_B0" "50.0" "$IPERF_WEIGHT_B_BW" \
    "${VETH_A0}=1" "${VETH_B0}=3" "${VETH_C0}=1" "${VETH_D0}=1"

# ── T4: Equal weights — path A (lowest RTT) must NOT dominate; expect ~25% ──
_T4_PASSED="false"; _T4_CHK_SHARE="0.0"; _T4_MBPS="0.0"; _T4_RESULT=""
run_weight_dist_test \
    "T4" "Equal split: all weights=1, target ${IPERF_EQUAL_BW} UDP UL" \
    "$VETH_A0" "25.0" "$IPERF_EQUAL_BW" \
    "${VETH_A0}=1" "${VETH_B0}=1" "${VETH_C0}=1" "${VETH_D0}=1"

# ── Generate JSON ──

OUTPUT_FILE="${CI_BENCH_RESULTS}/wrr_weight_$(date +%Y%m%d_%H%M%S).json"
...
def chk_share(d):       return d.get('check_share', 0.0)
def expected(d):        return d.get('expected_share', 0.0)
...
out = {
    ...
    'timestamp': '${TIMESTAMP}',
    'scheduler': 'wrr',
    'share_tolerance_pp': ${SHARE_TOLERANCE_PP},
    'netem': {
    ...
    'results': {
        ...
        't2_weight_ratio': {
            'description':   'UDP UL ${IPERF_WEIGHT_A_BW} — A (w=3) vs B/C/D (w=1), total=6',
            'iperf_proto':   'udp', 'target_bw': '${IPERF_WEIGHT_A_BW}',
            'weights':       'A=3 B=1 C=1 D=1',
            'throughput_mbps': float('${_T2_MBPS}'),
            'check_iface':   '${VETH_A0}',
            'check_share_pct': chk_share(t2),
            'expected_share_pct': expected(t2),
            'per_path_share_pct': {
        ...
        't3_weight_gt_rtt': {
            'description':   'UDP UL ${IPERF_WEIGHT_B_BW} — B (w=3, worst RTT) vs A/C/D (w=1), total=6',
            'iperf_proto':   'udp', 'target_bw': '${IPERF_WEIGHT_B_BW}',
            'weights':       'A=1 B=3 C=1 D=1',
            'throughput_mbps': float('${_T3_MBPS}'),
            'check_iface':   '${VETH_B0}',
            'check_share_pct': chk_share(t3),
            'expected_share_pct': expected(t3),
            'per_path_share_pct': {
        ...
        't4_equal_split': {
            'description':   'UDP UL ${IPERF_EQUAL_BW} — all weights=1, expect ~even split regardless of RTT',
            'iperf_proto':   'udp', 'target_bw': '${IPERF_EQUAL_BW}',
            'weights':       'all=1',
            'throughput_mbps': float('${_T4_MBPS}'),
            'check_iface':   '${VETH_A0}',
            'check_share_pct': chk_share(t4),
            'expected_share_pct': expected(t4),
            'per_path_share_pct': {
    ...
    'summary': {
        ...
        't2_weight_ratio':  passed(t2),
        't3_weight_gt_rtt': passed(t3),
        't4_equal_split':   passed(t4),
    },
}
...
print()
print(f"  {'Test':<30}  {'Check share':>18}  {'Result':>8}")
print(f"  {'-'*30}  {'-'*18}  {'-'*8}")
for tid, r in out['results'].items():
    chk = r.get('check_share_pct', '-')
    exp = r.get('expected_share_pct')
    chk_s = f"{chk:.1f}% (exp {exp:.0f}%)" if isinstance(chk, float) and exp is not None else str(chk)
    status = 'PASS' if r['passed'] else 'FAIL'
    gain = r.get('4path_gain_pct')
    extra = f' (+{gain:.1f}%)' if gain is not None else ''
    print(f"  {tid.upper():<30}  {chk_s+extra:>18}  {status:>8}")
print(f"  {'-'*30}  {'-'*18}  {'-'*8}")
print(f"  {'OVERALL':<30}  {'':>18}  {'PASS' if all_pass else 'FAIL':>8}")
print()
PYEOF
...
if not d['summary']['all_passed']:
    print('CI FAIL: one or more WRR weight tests failed')
    sys.exit(1)
"
